...
from typing import Any, Callable, Iterator, List, Mapping, Match, Optional, Union
...
def _cider_d_update(
    ...
    new_cooked_mrefs = [[__cook_sentence(ref, n, tokenizer) for ref in refs] for refs in mult_references]
...
def __compute_cider(
    ...
    for i, (cand, refs) in enumerate(zip(cooked_cands, cooked_mrefs)):
        ...
        for j, ref in enumerate(refs):
            vec_ref, norm_ref, length_ref = __counter_to_vec(ref, log_n_refs, n, doc_frequencies)
            vec_refs.append(vec_ref)
            ngrams_scores[j] = __similarity(vec, vec_ref, norm, norm_ref, length, length_ref, n, sigma)
