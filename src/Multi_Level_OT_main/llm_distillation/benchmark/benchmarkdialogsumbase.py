...
if __name__ == "__main__":
    ...
    if isinstance(dataset['summary'][0], dict): answers = [item[args.mapping_dict] for item in dataset['summary']]
    elif isinstance(dataset['summary'][0][0], dict): answers = [item[0][args.mapping_dict] for item in dataset['summary']]
    ...
    if args.save_predictions:
        prediction_data = [{'answers': dataset['summary'][index], 'prediction_text': item} for index, item in enumerate(predictions)]
